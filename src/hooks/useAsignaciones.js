...
export const generarAsignaciones = (ayerAsignaciones, tareasCompletadasAyer, usuariosN) => {
  ...
  // ── Carry-over de tareas DIARIAS no completadas ayer (al mismo usuario) ──
  // Si ayer se asignó una tarea diaria a un usuario y no la hizo, se le
  // vuelve a asignar hoy a él (no pasa a otro usuario).
  const carryoverDiariasIds = new Set();
  if (ayerAsignaciones) {
    diarias.forEach(t => {
      const usuarioAyer = Object.entries(ayerAsignaciones).find(([, ids]) => ids.includes(t.id))?.[0];
      if (usuarioAyer && usuariosN.includes(usuarioAyer) && !tareasCompletadasAyer.includes(t.id)) {
        asignaciones[usuarioAyer].push(t.id);
        carryoverDiariasIds.add(t.id);
      }
    });
  }

  // ── Repartir aleatoriamente solo las tareas diarias que SÍ se completaron ──
  const shuffled = shuffle(diarias.filter(t => !carryoverDiariasIds.has(t.id)));
  shuffled.forEach((t, i) => {
    asignaciones[usuariosN[i % usuariosN.length]].push(t.id);
  });

  // ── Carry-over de tareas SEGÚN NECESIDAD (lógica original sin cambios) ───
