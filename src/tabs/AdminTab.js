...
import { USER_ICONS, TASK_STATUS_CONFIG } from '../constants/ui_constants';
...
export const AdminTab = ({
  ...
  const [limpiarResultado,   setLimpiarResultado]   = useState(null);
  ...
  const confirmarRecalcular = () => setModalConfirm({
    visible: true,
    title: '🔄 Recalcular puntos',
    mensaje: 'Recalcula los puntos de todos los usuarios desde el historial verificado.\n\n¿Continuar?',
    onConfirm: onRecalcular,
  });

  const confirmarRepartir = () => setModalConfirm({
    visible: true,
    title: '🎲 Repartir tareas ahora',
    mensaje: 'Se repartirán aleatoriamente TODAS las tareas de casa entre los 3 usuarios para hoy, reemplazando el reparto actual.\n\n¿Continuar?',
    onConfirm: onRepartirAhora,
  });

  const confirmarResetearHoy = () => setModalConfirm({
    visible: true,
    title: '🗑️ Resetear tareas de hoy',
    mensaje: 'Se eliminarán TODOS los registros de hoy del historial. Las tareas quedarán como pendientes.\n\n¿Continuar?',
    onConfirm: onResetearHoy,
  });
  ...
  return (
    <View style={styles.tabContent}>
                    ...
                    {TAREAS_PERSONALES.map(t => {
                      ...
                      return (
                        <View key={t.id} style={styles.tareaFila}>
                          <Text style={[styles.tareaEstado, { color: TASK_STATUS_CONFIG[estado].color }]}>
                            {TASK_STATUS_CONFIG[estado].icono}
                          </Text>
                          <Text style={[
      ...
      {/* Modal limpiar usuarios duplicados */}
      <Modal
        visible={modalLimpiar}
        transparent
        animationType="fade"
        onRequestClose={() => setModalLimpiar(false)}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            {limpiarResultado === null ? (
              <>
                <Text style={styles.modalTitle}>🧹 Limpiar usuarios duplicados</Text>
                <Text style={styles.modalSub}>
                  Se eliminarán los documentos de usuario con IDs no reconocidos.{'\n'}
                  Solo se conservan: Daniel, Sergio, Diego y Adulto.
                </Text>
                <View style={styles.modalBtns}>
                  <TouchableOpacity
                    style={styles.modalBtnOk}
                    onPress={async () => {
                      const n = await onLimpiarUsuarios();
                      setLimpiarResultado(n);
                    }}
                  >
              ...
              <>
                <Text style={styles.modalTitle}>
                  {limpiarResultado === 0 ? '✅ Sin duplicados' : `✅ ${limpiarResultado} eliminados`}
                </Text>
                <Text style={styles.modalSub}>
                  {limpiarResultado === 0
                    ? 'No se encontraron usuarios duplicados.'
                    : `Se eliminaron ${limpiarResultado} documentos duplicados. La lista de usuarios ahora es correcta.`}
                </Text>
                <TouchableOpacity
                  style={styles.modalBtnOk}
                  onPress={() => setModalLimpiar(false)}
            ...
            <View key={u.id} style={styles.usuarioRow}>
              ...
              <View style={styles.usuarioRight}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => onResetearPuntosUsuario(u.id)}
                >
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity
          style={[styles.actionBtn, styles.actionBtnDanger, { marginTop: 16 }]}
          onPress={() => { setLimpiarResultado(null); setModalLimpiar(true); }}
        >
          <Text style={styles.actionBtnText}>🧹 Limpiar usuarios duplicados</Text>
        </TouchableOpacity>
        <Text style={styles.hint}>
          Elimina documentos de usuario con IDs no reconocidos (causados por el bug de addDoc anterior).
          Solo afecta a duplicados, nunca a Daniel / Sergio / Diego / Adulto.
        </Text>
      </View>

      {/* Modal de confirmación genérico (Repartir / Resetear / Recalcular) */}
      <Modal
